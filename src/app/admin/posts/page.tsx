          ...
          <div className="mt-6 grid gap-3">
            {posts.map((post) => (
              <div key={post.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-silver bg-white p-4">
                <div><b>{post.title}</b><p className="text-sm text-steel">{statusLabel(post.status)} | /blog/{post.slug}</p></div>
            ...
            {!posts.length ? <p className="rounded-2xl border border-silver bg-white p-5 text-sm leading-7 text-steel">هنوز مقاله‌ای ثبت نشده است. از فرم کنار صفحه اولین مقاله را اضافه کنید.</p> : null}
          </div>
        </section>
        <PostForm />
      </div>
...
function PostForm({ post }: { post?: BlogPost | null }) {
  return (
    <form action={savePostAction} className="grid h-fit gap-3 rounded-2xl border border-silver bg-white p-5 shadow-panel">
      ...
      <input name="title" required defaultValue={post?.title} placeholder="مثلاً قبل از خرید ماینر چه چیزهایی را بررسی کنیم؟" className="rounded-xl border border-silver px-3 py-2" />
      ...
      <input name="category" defaultValue={post?.category || ""} placeholder="مثلاً خرید ماینر" className="rounded-xl border border-silver px-3 py-2" />
      <textarea name="excerpt" required rows={3} defaultValue={post?.excerpt} placeholder="خلاصه کوتاه و قابل فهم برای کارت مقاله" className="rounded-xl border border-silver px-3 py-2" />
      <textarea name="content" required rows={10} defaultValue={post?.content} placeholder="متن کامل مقاله را با تیترهای واضح بنویسید" className="rounded-xl border border-silver px-3 py-2" />
