...
export default async function MinersPage() {
  ...
  return (
    <AdminShell>
      <div className="flex items-center justify-between gap-4"><h1 className="text-3xl font-extrabold">مدیریت ماینرها</h1><Link href="/admin/miners/new" className="rounded-xl bg-gold px-4 py-2 font-extrabold text-graphite">ماینر جدید</Link></div>
      <div className="mt-6 grid gap-3">
        {items.map((item) => (
          <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-silver bg-white p-4">
            <div><b>{item.title}</b><p className="text-sm text-steel">{statusLabel(item.status)} | {item.priceText || "قیمت روز با هماهنگی"}</p></div>
