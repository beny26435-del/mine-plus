...
export function ProductForm({ product, kind }: { product?: ProductInput | null; kind: "miner" | "part" }) {
  return (
    <form action={saveProductAction} className="grid gap-4 rounded-2xl border border-silver bg-white p-5 shadow-panel md:grid-cols-2">
      ...
      <label className="grid gap-1 font-bold">قیمت نمایشی<input name="priceText" defaultValue={product?.priceText || "قیمت روز"} className="rounded-xl border border-silver px-3 py-2" /></label>
      <label className="grid gap-1 font-bold">وضعیت موجودی<select name="stockStatus" defaultValue={product?.stockStatus || "inquiry"} className="rounded-xl border border-silver px-3 py-2"><option value="available">موجود</option><option value="inquiry">هماهنگی قبل از خرید</option><option value="unavailable">ناموجود</option></select></label>
