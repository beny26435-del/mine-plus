      ...
      <div className="container grid gap-8 md:grid-cols-[1.3fr_1fr_1fr]">
        <div>
          ...
          <p className="mt-5 max-w-md text-sm leading-8 text-silver">
            ماین پلاس برای خرید ماینر، تهیه قطعه، تعمیر دستگاه و آماده‌سازی فارم کنار شماست؛ با تمرکز روی انتخاب درست، نه تصمیم عجولانه.
          </p>
        </div>
        <div>
          ...
          <div className="grid gap-2 text-sm">
            ...
            <Link href="/farm-setup">درخواست مشاوره</Link>
            <Link href="/contact">تماس</Link>
          </div>
        </div>
        <div>
          <h3 className="mb-4 font-extrabold text-white">تماس</h3>
          <a href={telHref(contact.phone)} className="block font-bold text-white" dir="ltr">{contact.phone}</a>
          <a href={contact.whatsappLink} className="mt-3 inline-flex rounded-xl bg-gold px-4 py-2 text-sm font-extrabold text-graphite">پیام در واتساپ</a>
        </div>
      </div>
    </footer>
  );
